  ...
  return `<div class="main svelte-12hu38f">${function(__value) {
    ...
    return function(cont) {
      return ` ${each(cont.contant, ({ id, title, text, type }) => {
        return `${type == "test" ? `<div class="contant_box svelte-12hu38f"><img class="imag svelte-12hu38f" src="${escape(base, true) + "/icon.png"}" alt="사진"> <a href="${escape(base, true) + "/" + escape(type, true) + "/" + escape(id, true)}" class="title svelte-12hu38f">${escape(title)}</a><br> <p class="text svelte-12hu38f">${escape(text)}</p> </div>` : ``}`;
      })} `;
    }(__value);
  }(loading())} </div>`;
});
